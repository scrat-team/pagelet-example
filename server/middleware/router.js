...
module.exports = function (options, app) {
  return function mountRouters() {
    ...
    if(options.index){
      app.use(rewrite('/', options.index))
    }

    //auto mount controllers
    autoMount(app, options.root, options.deep || 5);
  ...
  walkDir(root, null, deep).forEach(function (item) {
    ...
    if (instance) {
      ...
      if (instance instanceof Router) {
        //when koa-router
        if (!instance.opts.prefix && mountPath !== '/') {
          instance.prefix(mountPath);
        }
        app.use(instance.routes());
        cluster.isMaster && logger.info('mount %s from /controller/%s', instance.opts.prefix || '/', relative);
      } else if (instance.constructor.name === 'GeneratorFunction') {
        //when middleware
        app.use(mount(mountPath, instance));
        cluster.isMaster && logger.info('mount %s from /controller/%s', mountPath, relative);
      } else {
